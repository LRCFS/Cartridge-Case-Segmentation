...
from configs.utils.image_preprocessing import correct_illumination

#My pre processing for RGB images (use for both training and testing!)
def my_PreProc_RGB(data, param=None):
    assert(len(data.shape)==4)
    data = data.transpose(0, 3, 1, 2)#(0, 2, 3, 1)
    assert (data.shape[1]==3)  #Use the original images
    #my preprocessing:
    #train_imgs = ben_preprocessing(data)
    # train_img = illumination_correction(data)
    train_img = histo_equalized_RGB(data)
    train_img = train_img / 255.  # Original
   # train_imgs = dataset_standardized(data, param)
    train_imgs = train_img.transpose(0, 2, 3, 1) #(0,3,1,2)
    return train_imgs
