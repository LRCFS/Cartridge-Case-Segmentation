...
Muthu Mookiah modified the class SegmentionInfer on 2025/04/14.
"""
...
from utils import visualize,gray2binary
...
class SegmentionInfer(InferBase):
	...
	def predict(self):
		...
		for path in predList:
			...
			print(self.analyze_name(path))

			cv2.imwrite(self.config.test_result_path_FP + self.analyze_name(path) + "_FP.png",
					binaryResultb0.astype(np.uint8))
			cv2.imwrite(self.config.test_result_path_BF + self.analyze_name(path) + "_BF.png",
					binaryResultb1.astype(np.uint8))
			cv2.imwrite(self.config.test_result_path_BG + self.analyze_name(path) + "_BG.png",
					binaryResultb2.astype(np.uint8))
