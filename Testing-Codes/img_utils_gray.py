...
def extract_patches(full_imgs, config):
    ...
    # patches = np.memmap('patchimgs.npy', dtype='float64', mode='w+',shape=(N_patches_tot,config.patch_height,config.patch_width,full_imgs.shape[3]))
...
def pred_to_patches(pred,config):
    ...
    # pred_images = np.memmap('predimgs.npy',dtype='float64',mode='w+',shape=(pred.shape[0],pred.shape[1],config.seg_num+1))
